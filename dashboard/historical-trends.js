...
(() => {
  ...
  function figure(rows, config, i) {
    ...
    return `<figure class="explained-chart" aria-labelledby="${titleId}" aria-describedby="${captionId}"><div class="chart-heading"><h3 id="${titleId}">${i}. ${config.title}</h3><p>${charts.date(rows[0].period)}–${charts.date(rows.at(-1).period)} · ${period==='annual'?'calendar years':period==='monthly'?'calendar months':'12-month windows'}</p></div>${charts.legend(config.series,rows,config.unit)}${charts.lineChart(rows,config.series,{...config,mode:period})}<figcaption class="chart-explanation" id="${captionId}"><p><strong>What it shows.</strong> ${config.shows}</p><p><strong>How it was obtained.</strong> ${config.method}</p><p><strong>Time window.</strong> ${windowText} Contributor counts use the union of keys within each window, never the sum of monthly counts.</p><p class="chart-source">Source: OpenAlex metadata frozen 16 September 2026; arXiv-linked works with Mathematics as their primary OpenAlex field, queried publication years 2010–2026. Analysis year/month comes from the arXiv ID. This is a selected population. <a href="historical_participation.json" download>Exact values and provenance</a>.</p></figcaption></figure>`;
  }
  function renderCharts() {
    if (!data) return;
    const rows = data[period];
    const primary = {
      title:'Papers per observed contributor · Pₜ/Aₜ', unit:'papers per contributor key', series:definition,
    ...
    el('historical-support').innerHTML = configs.map((c,i)=>figure(rows,c,i+2)).join('');
  ...
  function renderFindings() {
    ...
    el('historical-findings').innerHTML = `<p class="eyebrow">WHAT THE RECORDED DATA SHOW</p><h2>The long-run comparison, with its coverage limits</h2><p class="body-copy">Between ${a.period} and ${b.period}, papers per OpenAlex ID change from <strong>${num(a.ids.papers_per_author)} to ${num(b.ids.papers_per_author)} (${change(a.ids.papers_per_author,b.ids.papers_per_author)})</strong>. Using distinct names, the ratio changes from <strong>${num(a.names.papers_per_author)} to ${num(b.names.papers_per_author)} (${change(a.names.papers_per_author,b.names.papers_per_author)})</strong>.</p><p class="body-copy">Retained paper counts change by <strong>${change(a.papers,b.papers)}</strong>, compared with <strong>${change(a.ids.authors,b.ids.authors)}</strong> for observed IDs and <strong>${change(a.names.authors,b.names.authors)}</strong> for names. A higher P/A means paper counts grew faster than the corresponding contributor-key count.</p><p class="body-copy">Across those years, byline retention ranges from <strong>${pct(lo)} to ${pct(hi)}</strong>. ${num(data.diagnostics.duplicate_arxiv_id_groups)} additional arXiv paper-ID groups have ambiguous multiple-work links and are excluded; graph 4 includes this earlier selection step. These are changes among recorded contributor keys on selected papers. They do not establish a change in individual researchers’ productivity or an effect of AI.</p><p class="body-copy">Mean returned team size rises from <strong>${num(a.mean_team_size)} to ${num(b.mean_team_size)}</strong>. Full-count coauthored-paper appearances per key change from ${num(a.names.incidences_per_author)} to ${num(b.names.incidences_per_author)} for names, and ${num(a.ids.incidences_per_author)} to ${num(b.ids.incidences_per_author)} for IDs. The identity P/A = (I/A) ÷ (I/P) explains why collaboration matters to the ratio.</p><p class="chart-note">P/A is mean fractional paper credit on complete bylines. For example, one paper shared by three distinct keys gives P/A = 1/3. Mean full-count coauthored papers is I/A, where I counts author–paper links.</p>`;
  ...
  fetch('historical_participation.json').then(r=>{if(!r.ok)throw Error('Historical data unavailable');return r.json();}).then(d=>{
    if(d.annual.length!==16||d.monthly.length!==192||d.rolling12.length!==181)throw Error('Incomplete historical series');
    for(const k of ['annual','monthly','rolling12'])for(const r of d[k]){
      if(!Number.isSafeInteger(r.papers)||r.papers<0||r.papers>r.coverage.linked_papers)throw Error('Invalid paper counts');
      for(const m of ['names','ids'])if(r.papers && (!r[m].authors||Math.abs(r.papers/r[m].authors-r[m].papers_per_author)>1e-10))throw Error('Invalid denominator');
    }
    data=d;renderCharts();renderFindings();
